...
import { m as R } from "./bundle.manifests-EFrSp_KE.js";
const V = {
...
`);
  ...
  if (!t) {
    const n = (r ? a : a.map((i) => encodeURIComponent(i))).join(M(o));
    switch (o) {
      case "label":
        return `.${n}`;
      case "matrix":
  ...
  const l = G(o), c = a.map((n) => o === "label" || o === "simple" ? r ? n : encodeURIComponent(n) : O({
    allowReserved: r,
    name: e,
    value: n
  })).join(l);
  ...
  const c = Q(o), n = Object.entries(a).map(
    ([i, f]) => O({
      allowReserved: r,
  ...
  if (o)
    for (const a of o) {
      ...
      if (Array.isArray(i)) {
        e = e.replace(
          a,
          _({ explode: l, name: c, style: n, value: i })
        );
        continue;
      }
      if (typeof i == "object") {
...
}, ee = (r, t) => t ? !!(r.headers.has(t) || r.query?.[t] || r.headers.get("Cookie")?.includes(`${t}=`)) : !1, te = async ({
  security: r,
  ...t
}) => {
  for (const e of r) {
...
const re = () => ({
  ...
  let t = U(H(), r);
  const e = () => ({ ...t }), o = (f) => (t = U(t, f), e()), a = re(), l = async (f) => {
    const s = {
      ...t,
      ...f,
    ...
    if (u.ok) {
      ...
      const d = (s.parseAs === "auto" ? Z(u.headers.get("Content-Type")) : s.parseAs) ?? "json";
      let w;
      switch (d) {
        case "arrayBuffer":
        case "blob":
  ...
  console.info("[LightningPayments] entrypoint onInit"), R.forEach((e) => t.register(e)), r.consumeContext(L, async (e) => {
...
//# sourceMappingURL=entrypoint-BSPFVdqp.js.map
